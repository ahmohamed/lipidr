...
In this workflow, we will use lipidomics data for murine serum fed a normal or high-fat diet.
...
It is helpful to imagine SkylineExperiment object as a table with lipid molecules as rows and samples as columns. We can then easily subset this table by selecting to specific rows and columns. The general syntax is `d[rows, cols]`.

The example below we select the first 10 transitions and 10 samples. We can check the `rowData` and `colData`
...
We can also apply conditional selections (*indexing*). For example, we can select only QC samples.
...
We can also look at intensity and retention time distribution for each lipid molecule. It is usually helpful for check the variation across QC samples.
...
We can visualize differential analysis results using volcano plots. In this instance we turn off labeling, due to the large number of significantly changed lipids between conditions.
