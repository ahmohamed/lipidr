...
Datasets are represeted in R as `SummarizedExperiment`s to facilitate integration other Bioconductor packages.
...
For domenstration purposes, we select only 3 lipids classes, Ceramides (`Cer`), PhosphatidylCholines (`PC`) and LysoPhosphatidylCholines (`LPC`). We also `BileAcid` treated group from our dataset.
...
To ensure data quality, we can look at total lipid intensity as bar chart (`plot_sample_tic`) or distribution of samples (`plot_sample_boxplot`).

```{r, fig.height=6}
plot_sample_tic(d, log=TRUE)
```

We can also look at intensity and retention time distributions for each lipid molecule using `plot_molecule_boxplot`. It is recommended to assess the variation across quality control samples.
...
example `components = c(2,3)` plots 2nd and 3rd components.
