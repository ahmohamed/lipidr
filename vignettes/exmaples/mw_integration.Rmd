...
# Download datasets
Datasets can be easily downloaded and prased into `SkylineExperiment` object
using `lipidr` function `fetch_mw_study()` by supplying a `study_id`. The
exmaple shown in this vignette is from `ST001111` [link](https://www.metabolomicsworkbench.org/data/DRCCMetadata.php?Mode=Study&DataMode=AllData&StudyID=ST001111&StudyType=MS&ResultType=1). The dataset contains positive and netgative MS data for untargeted 
...
## Quality control
We look at total ion intensity for each sample.
```{r}
plot_samples(d, "tic")
```
...
```
We can see mild separation between benign and cancer samples, but not between cancer and metastatsis. We can also spot 2 outlier samples that we may want to consider checking closely. The low variace explained by `PC1` and `PC2`
(cumulative 25%, displayed in the plot as `R2X`) indicate highly variable lipid profiles in these clinical samples.
...
By quickly looking at the volcano plots, we can confirm the minute difference between cancer and metastatic samples. A fairly large difference is observed between cancer and benign samples, with PCs and PGs upregulated and CLs and TGs downregulated in cancer tissues.

## Multi-group comparison
Instead of two-group comparison, we might be interested in lipids are differetially expressed in any group.
We can perform ANOVA-style multigroup comparison using `de_design` function, which allows users to provide
...
In this exmaple, we will use cancer stage as our grouping variable. In our dataset, we can see samples with Stages I-IV. Using ANOVA-style analysis, we will identify all lipid molecules likely to be different among cancer stages.
...
Users interested in creating more complex design matrices are refered to (Limma User Guide)[https://www.bioconductor.org/packages/devel/bioc/vignettes/limma/inst/doc/usersguide.pdf] and (edgeR tutorial)[https://www.bioconductor.org/packages/devel/bioc/vignettes/edgeR/inst/doc/edgeRUsersGuide.pdf].
